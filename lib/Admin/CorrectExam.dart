...
class _CorrectExamState extends State<CorrectExam> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            children: [
              FutureBuilder<List<Exam>>(
                builder:(context, snapshot) {
                  return Text("Select question    Times left Exam: " + leftExam.toString() + "     ");
                },
              ),
              ElevatedButton(
                  onPressed: () {
      ...
      floatingActionButton: ElevatedButton(
          onPressed: () {
            studentExam.corrected = true;
            studentExam.score = score;
            studentExam.studentAnswers = examAnswers;

            studentExam.studentAnswers.forEach(
                  (element) =>
              score += StudentAnswer
                  .fromJson(element, '')
                  .score,
  ...
  Future openDialog(StudentAnswer answer) =>
      showDialog(
          context: context,
          builder: (context) =>
              AlertDialog(
                ...
                content: TextField(
                    ...
                    autofocus: true,
                    decoration:
                    const InputDecoration(hintText: 'Geef hier de score in')),
                actions: [
                  TextButton(
                      onPressed: () {
  ...
  Future<List<Exam>> getExam() async {
    ...
    var querySnapshot =
    await collection.where('studentId', isEqualTo: widget.student.id).get();
    List<Exam> result = [];
